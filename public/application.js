...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://cryptic-refuge-45274.herokuapp.com',
...
var successColor = '#d0e0e3';

var newTaskSuccess = function newTaskSuccess() {
  $('#message').text('Successfully created task');
  $('#message').css('background-color', successColor);
  showTasks();
};

var onAddNewTask = function onAddNewTask(event) {
  ...
  if (store.user) {
    ...
    console.log(data);
    api.addTask(data).then(newTaskSuccess).catch(ui.addTaskFailure);
  } else {
...
var displayAddNewTask = function displayAddNewTask(task) {
...
var updateTaskSuccess = function updateTaskSuccess() {
...
var onClickDone = function onClickDone() {
  ...
  if (!store.user) {
    ...
    api.updateTask(store.tasks[this.id].id, data).then(updateTaskSuccess).catch(ui.updateTaskFailure);
  }
};

var onUpdateTask = function onUpdateTask() {
  event.preventDefault();
  var data = getFormFields(event.target);
  ...
  if (!store.user) {
    ...
    api.updateTask(store.tasks[store.updatingTask].id, data).then(updateTaskSuccess).catch(ui.updateTaskFailure);
  }
};
...
var onClickEdit = function onClickEdit() {
  ...
  // draw edit ui
  $('#task-changes').html('');
  var editTaskForm = document.createElement('form');
  var editTaskButton = document.createElement('input');
  var taskField = document.createElement('input');
  taskField.setAttribute('type', 'text');
  taskField.setAttribute('name', 'task[text]');
...
var displayOneTask = function displayOneTask(task, id) {
  ...
  // console.log(task)
  var taskIdHTML = '<form class="form-inline" form id="task-';
  var taskTextHTML = '"> <div class="form-group mb-2"> <input type="text" readonly class="form-control-plaintext task" value=';
  var doneButtonHTML = '> </div> <button type="submit" class="btn btn-primary mb-2 done" id="';
  var editButtonHTML = '">Done</button><button type="submit" class="btn btn-primary mb-2 edit" id="';
  var postTaskHTML = '">Edit</button> </form>';
  var formId = '#task-' + id;
  var allTheHTML = taskIdHTML + id + taskTextHTML + task.text + doneButtonHTML + id + editButtonHTML + id + postTaskHTML;
  $('#task-area').append(allTheHTML);
  $(formId).on('click', '.done', onClickDone);
  $(formId).on('click', '.edit', onClickEdit);
};

var getAllTasksSuccess = function getAllTasksSuccess(data) {
...
var getTasks = function getTasks() {
  ...
  if (store.user) {
    // api call here
    api.getAllTasks().then(getAllTasksSuccess).catch(ui.getAllTasksFailure);
    return;
  }
  if (!store.tasks) {
...
var displayAllTasks = function displayAllTasks() {
...
var hideTasks = function hideTasks() {
  $('#task-area').html('');
  $('#task-changes').html('');
};

var onHideTasks = function onHideTasks() {
  event.preventDefault();
  hideTasks();
};

var displayHideTasksButton = function displayHideTasksButton() {
  // display "hide all tasks" button
  var hideTasksForm = document.createElement('form');
  var hideTasksButton = document.createElement('input');
...
var onShowTasks = function onShowTasks() {
  event.preventDefault();
  showTasks();
};
...
var addTask = function addTask(data) {
  return $.ajax({
...
var updateTask = function updateTask(id, data) {
  return $.ajax({
...
var newTaskFailure = function newTaskFailure() {
  $('#message').text('Failure creating task');
  $('#message').css('background-color', failureColor);
};

var getAllTasksFailure = function getAllTasksFailure() {
  $('#message').text('Failure getting tasks from api');
  $('#message').css('background-color', failureColor);
};

var updateTaskFailure = function updateTaskFailure() {
  $('#message').text('Failure updating task');
  $('#message').css('background-color', failureColor);
};

module.exports = {
  newTaskFailure: newTaskFailure,
  getAllTasksFailure: getAllTasksFailure,
